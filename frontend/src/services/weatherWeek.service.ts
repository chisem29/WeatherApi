...
export default class WeatherWeekService {
  lang?: string;
  mode?: 'json' | 'xml';
  apiKey: string;
  ...
  public async getData<T = number>(lat: T, lon: T) {
    const { data } = await axios<weatherRepI>({
      method: 'POST',
      url: `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${this.apiKey}&mode=${this.mode}`,
    });
  ...
  public async getLoc<T = string>(city: T) {
    const { data } = await axios<Array<locCountryI>>({
      method: 'GET',
      url: `http://api.openweathermap.org/geo/1.0/direct?q=${city}&limit=1&appid=${this.apiKey}&mode=${this.mode}`,
    });
